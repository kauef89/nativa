<template>
  <div class="flex flex-col h-full bg-surface-900 border-r border-surface-800">
    <div class="p-4 border-b border-surface-800 flex justify-between items-center bg-surface-950 shrink-0">
      ...
      <Button 
        ...
        size="small" 
        class="!bg-primary-600 hover:!bg-primary-500 !border-none !text-xs !font-bold !px-3 !py-2"
        @click="$emit('close')"
      />
    </div>
    ...
    <div class="flex-1 overflow-y-auto p-4 space-y-6 scrollbar-thin">
      ...
      <div v-else class="relative border-l-2 border-surface-700 ml-3 space-y-8 pb-4 pt-2">
        ...
        <div v-for="event in events" :key="event.id" class="relative pl-6 group">
          ...
          <div class="absolute -left-[9px] top-1 w-4 h-4 rounded-full border-2 border-surface-900 shadow-sm transition-transform group-hover:scale-125"
               :class="getEventColor(event.type)"></div>
          
          <div class="flex flex-col gap-1.5">
            <div class="flex items-center gap-2">
                <span class="text-[10px] font-bold font-mono text-surface-400 bg-surface-950 px-1.5 py-0.5 rounded border border-surface-800">
            ...
            <div class="text-sm text-surface-300 leading-snug">
              {{ event.description }}
            </div>
            ...
            <div v-if="event.type === 'swap'" class="bg-surface-800/50 p-3 rounded-lg border border-surface-700 mt-1 text-xs relative overflow-hidden">
                <div class="flex items-center gap-2 text-red-400 opacity-80 mb-1">
                    ...
                    <span class="line-through">{{ event.meta?.old_item || 'Item anterior' }}</span>
                </div>
                ...
                <div class="mt-2 pt-2 border-t border-surface-700/50 flex justify-between items-center">
                    <span class="text-[10px] uppercase font-bold tracking-wider" :class="getStatusColor(event.status)">
                        {{ getStatusLabel(event.status) }}
                    </span>
                    <span v-if="event.approver_name" class="text-[9px] text-surface-500 flex items-center gap-1">
            ...
            <div v-if="event.type === 'cancel' && event.status === 'pending'" class="bg-orange-500/10 p-2 rounded border border-orange-500/20 mt-1">
                <p class="text-xs text-orange-300 flex items-center gap-2">
                    <i class="fa-solid fa-hourglass-half"></i> Aguardando aprovação do gerente
                </p>
            </div>
...
// Props: Recebe a lista de eventos do backend/store
